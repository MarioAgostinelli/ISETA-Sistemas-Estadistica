---
title: "Probabilidad - Grandes números - Dos cumplen años el mismo día"
...
veces <- 10000
resultados <- replicate(veces, {    
    personas <- sample(dias, 25, replace = TRUE)
    any(duplicated(personas))
})
mean(resultados) 
```
...
```{r}
estima_probabilidad <- function(cantPersonas, veces = 10000){
    resultados <- replicate(veces, {
...
prob <- sapply(grupos, estima_probabilidad)
prob
```
